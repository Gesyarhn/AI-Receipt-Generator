...
class PantryItem {
    ...
    static async create(userId,itemData) {
        const { name, quantity, unit, category, expiry_date, is_runninglow = false } = itemData;
        const result = await db.query(
            `INSERT INTO pantry_items (user_id, name, quantity, unit, category, expiry_date, is_running_low) 
            VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
            [userId, name, quantity, unit, category, expiry_date, is_runninglow]
        );
    ...
    static async findByUserId(userId, filters = {}) {
        ...
        if (filters.is_runninglow !== undefined) {
            paramCount++;
            query += ` AND is_running_low = $${paramCount}`;
            params.push(filters.is_runninglow);
        }
    ...
    static async update(id, userId, updates) {
        const { name, quantity, unit, category, expiry_date, is_runninglow } = updates;
        ...
        const result = await db.query(
            ...
            RETURNING *`,
            [name, quantity, unit, category, expiry_date, is_runninglow, id, userId]
        );
        return result.rows[0];
    }
