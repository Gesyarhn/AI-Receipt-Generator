...
class ShoppingList {
    ...
    static async generateFromMealPlan(userId, startDate, endDate) {
        ...
        try {
            ...
            const result = await client.query(
                ...
                JOIN recipes ri ON mp.recipe_id = ri.recipe_id
                WHERE mp.user_id = $1 
                AND mp.meal_date >= $2 
                AND mp.meal_date <= $3
            ...
            pantryResult.rows.forEach(item => {
                ...
                pantryMap.set(key, { quantity: item.quantity, unit: item.unit });
            });

            //insert into shopping list (substract pantry quantities)
    ...
    static async create(userId, itemData) {
        const { ingredient_name, quantity, unit, category, from_meal_plan } = itemData;
    ...
    //Update shopping list item (e.g. mark as bought)
