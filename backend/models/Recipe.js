import pool from '../config/db.js'; // ← ganti 'db' jadi 'pool'
...
class Recipe {
        ...
        try {
            ...
            const recipeResult = await client.query(
                ...
                [
                    ...
                    JSON.stringify(instructions), // ← fix di sini
                    dietary_tags, user_notes, image_url
                ]
            );
            ...
            if (ingredients.length > 0) {
                ...
                ingredients.forEach(ing => {
                    ingredientParams.push(ing.name || ing.ingredient_name, ing.quantity, ing.unit);
                });
    ...
    static async findByUserId(userId, filters = {}) {
        ...
        if (filters.search) {
            query += ` AND (r.name ILIKE $${paramCount + 1} OR r.description ILIKE $${paramCount + 2})`;
    ...
    static async update(id, userId, update) {
        const {
            name, description, cuisine_type, difficulty,
            prep_time, cook_time, servings, instructions,
            dietary_tags, user_notes, image_url,
        ...
        const result = await pool.query(
            ...
            [
                name, description, cuisine_type, difficulty,
                prep_time, cook_time, servings,
                instructions ? JSON.stringify(instructions) : undefined,
    ...
    static async getStats(userId) {
        const result = await pool.query(
            `SELECT COUNT(*) AS total_recipes,
                COUNT(DISTINCT cuisine_type) AS unique_cuisine_types,
                AVG(cook_time) AS average_cook_time
             FROM recipes
             WHERE user_id = $1`,
            [userId]
        );
