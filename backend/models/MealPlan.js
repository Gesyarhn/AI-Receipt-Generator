...
class MealPlan {
    ...
    static async findByDate(userId, startDate, endDate) {
        const result = await db.query(
            `SELECT mp.id, mp.recipe_id, mp.meal_date::text as meal_date, mp.meal_type, mp.created_at, mp.updated_at, r.name as recipe_name, r.image_url, r.prep_time, r.cook_time
            FROM meal_plans mp
    ...
    static async getWeeklyPlan(userId, weekStartDate) {
        ...
        return await this.findByDate(userId, weekStartDate, endDate);
    }
